...
is_currently_on=`curl -s -H "Authorization: ${LD_API_TOKEN}" -H "Content-Type: application/json"  https://app.launchdarkly.com/api/v2/flags/default\?env\=${environment}\&filter\="tags:${service},filterEnv:${environment}" | jq ".items[0].environments.${environment}.on"`
